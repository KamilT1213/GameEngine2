...
public:
	...
	explicit Texture(const char* filepath, uint32_t wrapS = GL_REPEAT, uint32_t wrapT = GL_REPEAT, uint32_t minFilter = GL_LINEAR, uint32_t magFilter = GL_LINEAR, bool HDR = false); //!< Constructor which takes a path to an image file to be loaded
...
private:
	void init(uint32_t width, uint32_t height, uint32_t channels, unsigned char* data, bool isHDR, uint32_t wrapS, uint32_t wrapT, uint32_t minFilter, uint32_t magFilter); //!< Initialise the texture
};
