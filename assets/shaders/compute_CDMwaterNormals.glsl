...
layout(binding = 1, rgba16f) uniform image2D waterNormalImg;
...
void main()
{
	...
	vec3 displacement0 = (imageLoad(waterImg, pixel_coords + ivec2(-1, 0)).xyz - vec3(0.5)) * 25;
	vec3 displacement1 = (imageLoad(waterImg, pixel_coords + ivec2(1, 0)).xyz - vec3(0.5)) * 25;
	vec3 displacement2 = (imageLoad(waterImg, pixel_coords + ivec2(0, 1)).xyz - vec3(0.5)) * 25;
	vec3 displacement3 = (imageLoad(waterImg, pixel_coords + ivec2(0, -1)).xyz - vec3(0.5)) * 25;
	...
	vec3 displacement4 = (imageLoad(waterImg, pixel_coords + ivec2(-1, -1)).xyz - vec3(0.5)) * 25;
	vec3 displacement5 = (imageLoad(waterImg, pixel_coords + ivec2(1, 1)).xyz - vec3(0.5)) * 25;
	vec3 displacement6 = (imageLoad(waterImg, pixel_coords + ivec2(-1, 1)).xyz - vec3(0.5)) * 25;
	vec3 displacement7 = (imageLoad(waterImg, pixel_coords + ivec2(1, -1)).xyz - vec3(0.5)) * 25;
	...
	//Col = vec4(norm, 1.0);
	if (isnan(Col).x) Col = imageLoad(waterNormalImg, pixel_coords + ivec2(1, 0));
	imageStore(waterNormalImg, pixel_coords, Col);
}
