...
void main()
{
	...
	gSpecMetallicRoughness = vec4(vec3(tpTex(u_albedoMap).r,0.8,0.5),1.0);

	float f = clamp(dot(vec3(0, -1, 0), normal) * 2,0, 1);
	colour = mix(tpTex(u_albedoMap), tpTex(u_altAlbedoMap), f);// texture(u_albedoMap, texCoord);
}
